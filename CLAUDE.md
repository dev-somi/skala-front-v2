...
script/   - 바닐라 JS (nav.js, footer.js, calendarUtils.js, weekView.js, monthlyView.js, sidebarDate.js, schedulerData.js, scheduler.js, upDown.js, grade.js, bag.js, weatherAPI.js, statusBarTicker.js, realtimeInfo.js)
...
### 개인 스케줄러 대시보드 (`scheduler.html` + `script/scheduler*.js` + `css/scheduler.css`)

위 캘린더 3페이지(리로드 기반)와 **완전히 별개인 단일 페이지 앱**입니다. 좌측 대시보드 사이드바 +
우측 인터랙티브 캘린더를 한 화면에 두고, 페이지 리로드 없이 인메모리 상태로 동작합니다.

- **구조**: `<main class="scheduler-layout">`(2열 그리드, `css/scheduler.css`)
  - 좌측 `<aside class="scheduler-sidebar">`: My Class/My Plan 탭(오늘 타임라인 ↔ 오늘 과목 예·복습
    에디터) + 상시 노출 Daily Todo + 하단 Upcoming Class(오늘 이후 가장 가까운 수업 자동 계산).
  - 우측 `<section class="scheduler-calendar">`: 오늘 날짜 + Weekly/Monthly 토글 + 이전/다음 이동,
    주간은 기존 `.week-grid`/`.time-block` CSS를, 월간은 `.scheduler-month-table`(구 `#sched_tbl`
    스타일을 스코프 복제) + 기존 `.cal-day-cell`/`.cal-day-chip`을 재사용해 렌더링.
- **리로드 없음 / 헬퍼 재사용**: 기존 `weekView.js`/`monthlyView.js`는 재사용하지 **않고**(전용 id
  강결합 + 전체 리로드 방식), 순수 날짜 헬퍼 `script/calendarUtils.js`만 읽기 전용으로 재사용합니다.
  스케줄러 전용 렌더 함수가 인메모리 상태(현재 주/월 `cursor`, `viewMode`)로 prev/next·토글을 처리.
- **데이터 레이어 분리**: `script/schedulerData.js`(`window.SchedulerData`)가 `schedule.json` fetch/캐시와
  `localStorage` 읽기/쓰기(파싱 실패 방어 포함)를 전담하고, `script/scheduler.js`가 컨트롤러(렌더·이벤트)
  입니다. 로드 순서: `calendarUtils.js → schedulerData.js → scheduler.js → nav.js → footer.js`.
- **localStorage 데이터 모델**(모두 `skala.scheduler.*` 접두어):
  `todos.<YYYY-MM-DD>` = `[{id,text,done}]`, `plan.<YYYY-MM-DD>` = `{<과목명>:{preview,review}}`,
  `events` = `[{id,date,time,title}]`(개인 일정), `view` = `"weekly"|"monthly"`. 새로고침해도 유지됨.
- **수업 데이터는 읽기 전용**(`schedule.json`, 2026년 7~12월). 개인 일정만 사용자가 추가/삭제하며
  수업 일정 위에 겹쳐 표시됩니다. 주간 그리드는 기존 뷰와 동일하게 **월~토 6열(08~18시)**이라 일요일
  개인 일정은 월간 뷰에서만 보입니다(의도된 제약). 오늘 수업이 없으면(주말/휴일) 타임라인·예·복습은
  "오늘은 수업이 없어요" 빈 상태를 보여줍니다.
